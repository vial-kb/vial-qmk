...
void keyboard_pre_init_user(void) {
    // Initialize I2C early
    i2c_init();

    // Send SSD1306 display off command (0xAE) directly
    // OLED address is typically 0x3C (or 0x3D)
    uint8_t display_off_cmd[] = {0x00, 0xAE};  // Control byte + Display OFF command
...
enum pointing_device_mode current_mode = MODE_MOUSE;
int actuation = 256;
// showing_actuation and timer are defined below in file already, checking...
// Restoring definitions explicitly here to ensure linkage
...
// --- SCREENSAVER LOGIC (Ported from Southpaw) ---
...
// Splash state (splash_timer removed - no longer using warp startup)
...
// Startup state - glitch -> noise -> dissolve to GUI
...
#define NOISE_DURATION 1000   // Animated static for 1 second
#define NOISE_FRAME_MS 50     // Update noise every 50ms (20fps)
#define DISSOLVE_MS 2500      // Slower fade duration (2.5 seconds)

// Simple random for noise pattern
...
// Render GUI with sparse noise dissolve overlay
// noise_level: 0 = full GUI, 100 = full noise
static void render_dissolve(uint8_t noise_level) {
    uint8_t width = oled_max_chars() * OLED_FONT_WIDTH;
    uint8_t height = oled_max_lines() * OLED_FONT_HEIGHT;

    // First render the GUI
    oled_clear();
    render_layer_state();

    // Then overlay very sparse noise pixels based on noise_level
    for (uint8_t y = 0; y < height; y++) {
        for (uint8_t x = 0; x < width; x++) {
            if ((fast_rand() % 100) < noise_level) {
                // Replace with sparse noise (~10% lit)
...
bool oled_task_user(void) {
        ...
        switch (startup_phase) {
            case 0:
                ...
                if (elapsed < NOISE_DURATION) {
                    ...
                    startup_phase = 1;
                    oled_startup_timer = timer_read32();  // Reset for fade
                }
                break;

            case 1:
                // Phase 1: Fade from noise to GUI with GUI flashing through
                if (elapsed < DISSOLVE_MS) {
                    // Quadratic ease-in: progress^2
                    uint32_t progress = (elapsed * 100) / DISSOLVE_MS;  // 0-100
                    uint32_t eased = (progress * progress) / 100;        // 0-100 squared/100
                    uint8_t noise_level = 100 - eased;

                    // Flash GUI through the noise randomly
                    // Use random chance each frame for irregular flashing
                    if ((fast_rand() % 8) == 0) {
                        // Random flash - show GUI with less noise
            ...
            case 2:
                // Phase 2: Startup complete - show clean GUI
    ...
    // Normal display logic - check if we should show actuation
