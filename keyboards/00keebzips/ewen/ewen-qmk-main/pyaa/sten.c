...
bool send_steno_chord_user(steno_mode_t mode, uint8_t chord[6]) { 
	...
	// Combo PWR and FN press, toggle QWERTY
#ifndef ONLYQWERTY
	if (cChord == (PWR | FN)) {
	...
	// Do QWERTY (removed momentary QWERTY)
	if (cMode == QWERTY || (cChord & (FN | PWR))) {
		processChord(false);
		goto out;
	}
...
bool process_steno_user(uint16_t keycode, keyrecord_t *record) { 
	...
	switch (keycode) {
			...
			case STN_N1...STN_N4:	pr ? (cChord |= (LNO)): (cChord &= ~(LNO)); break;
			case STN_N5:			pr ? (cChord |= (MNO)): (cChord &= ~(MNO)); break;
			case STN_N6:			pr ? (cChord |= (NNO)): (cChord &= ~(NNO)); break;
...
void processChord(bool useFakeSteno) {
	...
	while (savedChord != mask) {
		...
		for (int i = 0; i <= chordIndex; i++) {
			...
			// Lock EXTN layer in once detected
			if (mask & PWR)
				cChord |= PWR;
