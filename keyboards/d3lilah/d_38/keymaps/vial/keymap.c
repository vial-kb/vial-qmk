...
enum custom_keycodes {
    ...
    GDOC_H3,
    GDOC_NORMAL,
    KC_AC_TOGG  // Autocorrect toggle keycode
};
...
void keyboard_post_init_user(void) {
    ...
    // Report autocorrect status on startup
    if (autocorrect_is_enabled()) {
        uprintf("Autocorrect initialized: ON\n");
    } else {
        uprintf("Autocorrect initialized: OFF (press AC_TOGG to enable)\n");
    }
}
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    if (record->event.pressed) {
        switch (keycode) {
            ...
            // ========== AUTOCORRECT TOGGLE ==========
            case KC_AC_TOGG:
                // Toggle autocorrect - this calls QMK's internal toggle
                autocorrect_toggle();
                // Debug: Print current autocorrect state
                if (autocorrect_is_enabled()) {
                    uprintf("Autocorrect: ON\n");
                } else {
                    uprintf("Autocorrect: OFF\n");
                }
